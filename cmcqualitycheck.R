# this part is determining the quality of input data based on number of experiments,minimum values,maximum values,number of repeated values,number of missing values and then procude a summary table

# define a variable that contains 60 cancer cell line names
...
cmcqualitycheck <- function(dataset,counts){
	...
	# check if the dataset has 60 cancer cell lines and if the order and name are the same
	if ((dim(dataset)[1] != 60) | (identical(name,rownames(dataset)) == FALSE)) {
		 report[6,] <- ("Error: data format is not correct") 
	}
	
	# check if input data passed the pre-defined threshold, if not, show warning messages
